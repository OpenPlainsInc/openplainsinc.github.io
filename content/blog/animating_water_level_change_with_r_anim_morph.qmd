...
# Zoom to the water of either date, with a 500 m margin
tools.r_mapcalc(expression="lwater_any = if(water_lb == 1 || water_la == 1, 1, null())")
tools.g_region(zoom="lwater_any", res=10)
reg = tools.g_region(flags="p", format="json")
tools.g_region(n=reg["north"] + 500, s=reg["south"] - 500,
               e=reg["east"] + 500, w=reg["west"] - 500)

# 1-99 percentile stretch per band: red 78-3370, green 144-3349, blue 90-3337
bounds = {"red": (78, 3370), "green": (144, 3349), "blue": (90, 3337)}
for band, (lo, hi) in bounds.items():
    for t in ("lb", "la"):
        tools.r_mapcalc(
...
subprocess.run(
    ["ffmpeg", "-framerate", "15", "-i", "lake_%02d.png",
     "-vf", "scale=-2:1440:flags=lanczos", "-c:v", "libx264",
     "-pix_fmt", "yuv420p", "-crf", "23", "falls_lake_whole_anim_morph.mp4"],
    check=True,
)
```
...
g.region zoom=lwater_any res=10
eval $(g.region -g)
g.region n=$((n+500)) s=$((s-500)) e=$((e+500)) w=$((w-500)) -p
...
for T in lb la; do
    stretch ${T}_red    78 3370 ${T}_red8
    stretch ${T}_green 144 3349 ${T}_green8
    stretch ${T}_blue   90 3337 ${T}_blue8
done
...
ffmpeg -framerate 15 -i lake_%02d.png -vf "scale=-2:1440:flags=lanczos" \
    -c:v libx264 -pix_fmt yuv420p -crf 23 falls_lake_whole_anim_morph.mp4
```

:::

Over the whole reservoir the change is no longer subtle. **29.4 km² of water
on 26 July 2025 became 19.4 km² on 1 July 2026, a 34% loss**: 10.9 km² of
former lake surface was exposed while 1.0 km² was newly counted as water. The
...
which is why they animate first and longest. Most of the newly counted water
is cloud shadow on the July 2026 scene, which NDWI reads as water; masking
with the scene classification asset would remove it, and the same scattered
cumulus is visible in the frames.

:::: {layout-ncol=2}
![Frame 24 of 48 over the whole reservoir, 26 July 2025 morphing into 1 July 2026. The northwest reaches are already lakebed while the main body holds.](images/falls_lake_whole_frame_24.jpg){#fig-whole-frame fig-alt="True color satellite view of all of Falls Lake mid-way through the drawdown animation, with exposed lakebed in the upper reaches"}
...
The two dates bracket a real drought, not a seasonal cycle. Falls Lake is a
...
Set against that, the 9.4% surface-area loss over the mapped arms is a window,
not the whole story. Across the reservoir the same method finds a 34% loss of
