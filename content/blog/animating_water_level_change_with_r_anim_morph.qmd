...
### The same step from Python

r.anim.morph is a regular GRASS tool, so it is also callable from Python
through the `grass.tools` API (GRASS 8.5 and newer). Open the project, call
the tool as a method with the same options, and read the plan statistics
back as JSON:
...
with gs.setup.init(project) as session:
    ...
    tools.g_region(n=253700, s=249290, e=639420, w=634870, res=10)
    tools.r_anim_morph(
        before="bef_red8,bef_green8,bef_blue8",
        after="aft_red8,aft_green8,aft_blue8",
        output="fl",
...
Flags are passed as a string, for example `flags="c"` to match the before
bands to the after bands, and `overwrite=True` replaces existing frames. The
frame export in the next section works the same way: build a `Tools` object
with the `GRASS_RENDER_*` variables in its `env` and call `d_rgb` in a loop.
...
Set against that, the 9.4% surface-area loss over the mapped arms is the
understated number. Surface area is what an optical satellite can see, and it
lags stage badly on a reservoir like this one: a six-foot drop in the pool
took storage down by more than a third while removing less than a tenth of
the water surface in these arms. What area does capture well is *where* the
change happened. The upper arms shelve gently, so the stage drop exposes a
wide, continuous band of lakebed along every cove, and that is what r.anim.morph
