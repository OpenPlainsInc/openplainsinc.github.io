...
* `openplains.scss`: Brand SCSS theme for the website (palette, self-hosted Lexend Deca and Cascadia Mono from `fonts/`, navbar, listing cards, home page blocks). `theme.scss` is the older theme, now unused by any page; `content/themes/grass.scss` is the GRASS theme for GRASS-branded pages.
